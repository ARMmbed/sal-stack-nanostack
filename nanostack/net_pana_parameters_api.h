...
typedef struct pana_lib_parameters_s {
    ...
    uint16_t AUTHENTICATION_TIMEOUT;        /**< Max Timeout for Auhtencication timeout, Default 100 seconds */
    uint16_t KEY_UPDATE_THRESHOLD;      /**< Gap in seconds when server start send new Network key, Default 10 */
    ...
    uint8_t  AUTH_COUNTER_MAX;          /**< Define Pana Session Re-authentication limit when MAX value is reached Server not respond Pana Notify Request . Default 0xff*/
